...
const txtTail = document.getElementById("txtTail");
const checkUseLunar = document.getElementById("checkUseLunar");
const LunarOptions = document.getElementById("LunarOptions");
const checkLunarTail = document.getElementById("checkLunarTail");
const txtLunarYears = document.getElementById("txtLunarYears");
const butDownload = document.getElementById("butDownload");
const txtCustomProperty = document.getElementById("txtCustomProperty");
checkUseLunar.addEventListener("change", function (ev) {
    LunarOptions.style.display = this.checked ? "block" : "none";
});
function ParseICSEventData(txt) {
    if (txt.length < 4) {
        return [];
    }
    txt += "\n";
