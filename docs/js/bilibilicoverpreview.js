...
(function() {
    ...
    if (h == null) { throw "uploadimg is null" }

    function loadimgfile(f) {
        var reader = new FileReader;
        reader.readAsDataURL(f);
        reader.onload = function(ev) { if (this.result) { uploadCover.src = this.result.toString() } }
    }
    var input = h;
    input.onchange = function() {
        if (input.files != null) {
            var uploadimg = input.files.item(0);
    ...
    if (h == null) { throw "#videotitle is null" }
    h.onchange = RefreshPreview;
    document.addEventListener("paste", function(ev) {
        if (ev.clipboardData == null || ev.clipboardData.files == null) { return }
        var f = ev.clipboardData.files.item(0);
        if (f == null || f.type.indexOf("image") < 0) { return }
        loadimgfile(f)
    })
})();
...
function DrawTextInBlock(ctx, text, x, y, lineheight, maxwidth, maxlines) {
    ...
    while (text.length > 0) {
        ...
        var i = 0;
        for (i = 0; i < text.length; i++) { var m = ctx.measureText(text.substring(0, i + 1)); if (m.width > maxwidth) { break } }
...
function RefreshPreview() {
    if (uploadCover.src.length < 1) { return }
    var h;
    h = document.getElementById("videotitle");
    if (h == null) { throw "#videotitle is null" }
    var input = h;
    ...
    if (h == null) { throw "canvas #drawing is null" }
    var canvas = h;
    var ctx = canvas.getContext("2d");
    if (ctx == null) { throw "canvas 2d context is null" }
    canvas.height = canvas.height;
    var bgtype = "app";
    document.getElementsByName("bgtype").forEach(function(e) { var ie = e; if (ie.checked) { bgtype = ie.value } });
